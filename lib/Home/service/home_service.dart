...
enum EnumCityName { Moscow, NewYorkCity, Istanbul, London, Paris, Tokyo }
...
class HomeService {
  ...
  final String _urlOtherDays =
      "http://api.openweathermap.org/data/2.5/forecast?id=524901&appid=165eea882ce94482baf85e80def4b1d1";
  ...
  Future<HomeModel?> getDataFromIPAWithEnum(EnumCityName enumCityName) async {
    try {
      final response = await dio.get(
          "http://api.openweathermap.org/data/2.5/forecast?id=${enumCityName.ids()}&appid=165eea882ce94482baf85e80def4b1d1");
      if (response.statusCode == 200) {
        return HomeModel.fromJson(response.data);
      } else {
        print('${response.statusCode} : ${response.data.toString()}');
