  ...
  Widget weatherCards(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: const Color.fromARGB(255, 74, 142, 219),
...
class DateTextWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(
        "${DateFormat('MMMM').format(DateTime.parse(snapshot.data!.list[8 * index].dtTxt.toString()))}.${snapshot.data!.list[8 * index].dtTxt.day}${haveYear ? (", ${snapshot.data!.list[8 * index].dtTxt.year}") : ""}");
  }
}
...
class TimeText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(
      "${snapshot.data!.list[1].dtTxt.hour}:${snapshot.data!.list[0].dtTxt.minute}0 AM",
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return ListTile(
            ...
            subtitle: Text(
                "${DateFormat('MMMM').format(DateTime.parse(snapshot.data!.list[0].dtTxt.toString()))}.${snapshot.data!.list[0].dtTxt.day}, ${snapshot.data!.list[0].dtTxt.year}"),
          );
        }
      },
    );
...
class DailyWeather extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: const Color.fromARGB(255, 195, 214, 235),
